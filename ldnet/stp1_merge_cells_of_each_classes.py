...
def merge_cells_of_each_classes(labeled_cells, merged_cells):
    ...
    sub_dirs = [x for x in os.listdir(labeled_cells)]

    # sub_dir包含三个子目录 对应该图片的三个类 每个子目录中存放对应类的cells
    for sub_dir in sub_dirs:
        for class_dir in list_of_classes:
            # 从根目录到当前图片的一个类的路径
            root_to_one_class_of_current_img = labeled_cells + "/" + sub_dir + "/" + class_dir
...
def main(_):
    ...
    merge_cells_of_each_classes(labeled_cells="dir1_labeled_cells/" + train_or_validation[flag],
                                merged_cells="dir1_merged_cells/" + train_or_validation[flag])
