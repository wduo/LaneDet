...
def _generate_image_and_label_batch(result, batch_size, min_queue_examples, shuffle):
    ...
    if shuffle:
        images, label_batch, key_batch = tf.train.shuffle_batch(
            [result.float_image, result.label, result.key],
...
def main(_):
    images, labels, keys = generate_batches_from_tfrecords(records_name="ldnet_validation.tfrecords",
                                                           train_or_validation="validation")
    print(images, labels, keys)
